	    	...
	    	<div class="row">
	    		...
	    		<div class="cell-3">
                    <select name="role" data-role="select">
					    ...
					    <option value="0" {{ $user->role==0 ? "selected" : "" }}>{{ trans('cruds.user.roles.disabled') }}</option>
					</select>
				</div>
			</div>
			@endif
    		...
    		<div class="cell-8">
	            <select data-role="select" name="controls[]" id="controls" data-filter="true" multiple>
	                @foreach($measures as $measure)
                       <option value="{{ $measure->id }}" {{ (in_array($measure->id, old('controls', [])) || ($user->measures->contains($measure->id))) ? 'selected' : '' }}>
                       {{ $measure->name }}
                       </option>
	                @endforeach
	            </select>
