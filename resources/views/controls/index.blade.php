@extends("layout")

@section("content")
<div data-role="panel" data-title-caption="{{ trans('cruds.measure.index') }}" data-collapsible="false" data-title-icon="<span class='mif-books'></span>">
		<div class="grid">
            <div class="row mb-3">
				<div class="cell-5">
					<select id='domain_id' name="domain_id" data-role="select">
					    <option value="0">-- {{ trans('cruds.domain.choose') }} --</option>
						@foreach ($domains as $domain)
					    	<option value="{{ $domain->id }}"
								@if (((int)Session::get("domain"))==$domain->id)
									selected
								@endif >
					    		{{ $domain->title }}
					    	</option>
					    @endforeach
					</select>
				</div>
				<div class="cell-7" align="right">
                    <a class="button primary" href="/alice/create">
			            <span class="mif-plus"></span>
			            &nbsp;
						{{ trans('common.new') }}
                    </a>
				</div>
			</div>

		<script>
			window.addEventListener('load', function(){
		    var select = document.getElementById('domain_id');

		    select.addEventListener('change', function(){
		        window.location = '/alice/index?domain=' + this.value;
		    }, false);
		}, false);
		</script>

		<div class="row">
			<div class="cell">
            <table
                id="measures"
                class="table data-table striped row-hover cell-border"
                data-role="table">
		   <thead>
			    <tr>
					<th class="sortable-column sort-asc" width="10%">{{ trans('cruds.measure.fields.domain') }}</th>
					<th class="sortable-column sort-asc" width="10%">{{ trans('cruds.measure.fields.clause') }}</th>
					<th class="sortable-column sort-asc" width="60%">{{ trans('cruds.measure.fields.name') }}</th>
					<th class="sortable-column sort-asc" data-cls-column="text-center" width="10%"># {{ trans('cruds.control.title') }}</th>
					<th width="10%"></th>
			    </tr>
		    </thead>
		    <tbody>
		@foreach($controls as $control)
			<tr>
				<td>
					<a id="{{ $control->title }}" href="/domains/{{$control->domain_id}}">
						{{ $control->title }}
					</a>
				</td>
				<td>
                    <a id="{{ $control->clause }}" href="/alice/show/{{ $control->id }}">
					@if (strlen($control->clause)==0)
						None
					@else
						{{ $control->clause }}
					@endif
					</a>
				</td>
				<td>{{ $control->name }}</td>
				<td id="{{ $control->control_count }}">
					@if ($control->control_count>0)
                    <a href="/bob/index?period=99&domain=0&scope=none&status=2&clause={{ htmlentities($control->clause) }}">
						{{ $control->control_count }}
					</a>
					@else
					0
					@endif
				</td>
				<td>
				@if (!Auth::User()->isAuditee())
                    <a href="/alice/plan/{{ $control->id }}" class="button info small">
			            <span class="mif-calendar"></span>
			            &nbsp;
				    	{{ trans('common.plan') }}
                    </a>
                @endif
				</td>
			</tr>
		@endforeach
		</tbody>
		</table>
	</div>
</div>
@endsection
