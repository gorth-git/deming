...
@section("content")
<div data-role="panel" data-title-caption='{{ trans("cruds.measure.edit")}}' data-collapsible="false" data-title-icon="<span class='mif-paste'></span>">
		...
		<div class="grid">
	    	...
	    	<div class="row">
	    		...
	    		<div class="cell-lg-6 cell-md-10">
                    <select id="controls" name="controls[]" data-role="select" data-filter="true" multiple>
						@foreach($all_controls as $control)
						    <option value="{{ $control->id }}"
                                {{ in_array($control->id, old("controls", $controls)) ? "selected" : "" }}>
	    	...
	    	<div class="row">
                ...
                <div class="cell-lg-4 cell-md-6">
					<input type="text" data-role="input" name="name" value="{{ $measure->name }}" maxlength="255">
				</div>
				...
				<div class="cell-lg-6 cell-md-10">
                    <select data-role="select" id="attributes" data-filter="true" name="attributes[]" multiple>
						@foreach($attributes as $attribute)
						    ...
						    <option {{ str_contains($measure->attributes, $attribute) ? "selected" : ""}}>{{$attribute}}</option>
						    @endif
					    @endforeach
					 </select>
				</div>
	    	...
	    	<div class="row">
				...
				<div class="cell-lg-2 cell-md-3">
					...
					value="{{ $measure->realisation_date == '0000-00-00' ? null : $measure->realisation_date}}"
	    	...
	    	<div class="row">
                ...
                <div class="cell-lg-6 cell-md-10">
					<input type="radio" name="score" value="3" data-role="radio" data-append="<font color='green'>{{ trans('common.green') }}</font>" {{ ($measure->score==3) ? 'checked' : '' }}/>
					<input type="radio" name="score" value="2" data-role="radio" data-append="<font color='orange'>{{ trans('common.orange') }}</font>" {{ ($measure->score==2) ? 'checked' : '' }}/>
					<input type="radio" name="score" value="1" data-role="radio" data-append="<font color='red'>{{ trans('common.red') }}</font>" {{ ($measure->score==1) ? 'checked' : '' }}/>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="cell-lg-1 cell-md-2">
                    <select data-role="select" id="status" name="status">
						<option value="0" {{ $measure->status==0 ? "selected" : ""}}>Todo</option>
						<option value="1" {{ $measure->status==1 ? "selected" : ""}}>Proposed</option>
						<option value="2" {{ $measure->status==2 ? "selected" : ""}}>Done</option>
					 </select>
	    		</div>
                <div class="cell-lg-1 cell-md-2" align="right">
				...
				<div class="cell-lg-1 cell-md-2">
                    <select data-role="select" id="next_id" name="next_id">
						<option></option>
						@foreach($ids as $id)
						    <option {{ $measure->next_id === $id ? "selected" : ""}}>{{$id}}</option>
					    @endforeach
					 </select>
				</div>
			</div>
	    	...
	    	<div class="row">
                <div class="col-12">
    				...
    				&nbsp;
                    <a class="button" href="/bob/show/{{$measure->id}}" role="button">
